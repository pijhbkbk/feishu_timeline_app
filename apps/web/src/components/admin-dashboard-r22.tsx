  ...
  return (
    <div className="r22-page r22-admin-page" data-testid="admin-page">
      ...
      {data ? <>
        ...
        <section className="r22-card r22-admin-anomalies"><div className="r22-section-heading"><div><p className="r22-overline">Audit</p><h2>近期异常与敏感动作</h2><p>只展示真实审计日志中的失败、拒绝、删除和锁定事件。</p></div><R22StatusBadge tone={data.summary.anomalyCount > 0 ? 'warning' : 'success'}>{data.summary.anomalyCount > 0 ? '需要检查' : '运行正常'}</R22StatusBadge></div>{data.anomalies.length ? <div className="r22-activity-list">{data.anomalies.map((item) => <article key={item.id}><span className="r22-activity-dot" /><div><strong>{item.summary}</strong><p>{item.action} · {item.actorName}</p></div><time>{new Date(item.createdAt).toLocaleString('zh-CN')}</time></article>)}</div> : <div className="r22-empty-compact"><strong>近 30 天没有异常动作</strong><p>审计日志仍会持续记录关键写操作。</p></div>}</section>
      </> : null}
    </div>
  );
}
