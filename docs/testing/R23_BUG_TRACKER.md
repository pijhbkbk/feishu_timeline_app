...
- R23C 首次为只读请求补充请求 ID 时，k6 `setup()` 中不存在 `__ITER`；脚本已使用安全默认值修复，失败尝试已标记 INVALIDATED。
- 注销清理脚本最初只接受 200/204，未接受 NestJS `POST` 默认 201，导致已注销会话被误报失败；Nginx 证实返回 201，脚本已修正。
